...
export default function SignUp({ redirectTo }: { redirectTo: string }) {
	...
	return (
		<div
			className={` whitespace-nowrap p-5 space-x-0 overflow-hidden justify-end items-center align-top    ${
			...
			>
				<div className="flex h-full items-center justify-center flex-col space-y-5">
					...
					<div className="text-sm flex gap-2">
						<p>{"Didn't work?"} </p>
						<span
							className="text-background cursor-pointer hover:underline transition-all flex items-center gap-2 "
							onClick={async () => {
								if (!isSendAgain) {
